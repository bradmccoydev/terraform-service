...
const ServiceName = "terraform-service"
...
func processKeptnCloudEvent(ctx context.Context, event cloudevents.Event) error {
	...
	**/
	...
	// Every keptn task has a lifecycle:
	// 1. taskname.triggered is send by the shipyard controller (Remember: You trigger sequences, keptn handles the tasks)
	// 2. (mandatory) taskname.started (your service (ie. this code) MUST respond with a taskname.started event)
	// 3. (optional) if your task is long running and you want to keep keptn updated DURING task execution, use the taskname.status.changed event
	// 4. (mandatory) taskname.finished (your service ie. this code) MUST response with a taskname.finished event)

	// Typically you'd subscribe to .triggered to "do" something
	// Typically you'd subscribe to .started and / or .finished to "notify" something
	// For example: to run a load test, you might listen to lt.triggered
	// But to notify a messaging tool that you'd started the load test, you might listen to lt.started
	// To notify a messaging too when you have finished the lt you may listen for lt.finished
...
func _main(args []string, env envConfig) int {
	...
	log.Println("Starting terraform-service...")
