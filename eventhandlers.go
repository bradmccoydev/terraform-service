...
func HandleHelloTriggeredEvent(myKeptn *keptnv2.Keptn, incomingEvent cloudevents.Event) error {
	...
	if startedErr != nil {
		errMsg := fmt.Sprintf("Failed to send task started CloudEvent (%s), aborting...", startedErr.Error())
		log.Println(errMsg)
		return startedErr
	}
...
// HandleGetSliTriggeredEvent handles get-sli.triggered events if SLIProvider == terraform-service
...
func HandleGetSliTriggeredEvent(myKeptn *keptnv2.Keptn, incomingEvent cloudevents.Event, data *keptnv2.GetSLITriggeredEventData) error {
	...
	if data.GetSLI.SLIProvider != "terraform-service" {
	...
	// Get SLI File from terraform-service subdirectory of the config repo - to add the file use:
	//   keptn add-resource --project=PROJECT --stage=STAGE --service=SERVICE --resource=my-sli-config.yaml  --resourceUri=terraform-service/sli.yaml
	sliFile := "terraform-service/sli.yaml"
